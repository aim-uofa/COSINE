...
class DAVIS(object):
    ...
    def _check_directories(self):
        if not os.path.exists(self.root):
            raise FileNotFoundError(f'DAVIS not found in the specified directory {self.root}, download it from {self.DATASET_WEB}')
        if not os.path.exists(os.path.join(self.imagesets_path, f'{self.subset}.txt')):
            raise FileNotFoundError(f'Subset sequences list for {self.subset} {os.path.join(self.imagesets_path, f"{self.subset}.txt")} not found, download the missing subset '
...
if __name__ == '__main__':
    ...
    for s in subsets:
        dataset = DAVIS(root='datasets/vos/DAVIS/2017/trainval', subset=s)
