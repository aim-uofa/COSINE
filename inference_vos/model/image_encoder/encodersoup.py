    ...
    def encode_prompt(self, batched_visual_prompt, batched_text_prompt, batched_tag):
        ...
        for vis_prompt, text_prompt, tag in zip(batched_visual_prompt, batched_text_prompt, batched_tag):
            if tag == 'refer':
                ...
                ref_embeddings = []
                for batch_images_tensor in batch_iterator(4, images.tensor):
                    with torch.no_grad():
                        ref_embedding = self.dinov2.get_enc_embs(batch_images_tensor[0])
                    ref_embedding = self.dinov2.neck(ref_embedding)
                    ref_embeddings.append(ref_embedding)
                ref_embedding = torch.cat(ref_embeddings, dim=0)
