...
from inference_vos.model.image_encoder.encodersoup_ms import build_encodersoup

from cosine.model.pixel_decoder.msdeformattn import build_pixel_decoder
from inference_vos.model.transformer_decoder.mformer_ms import build_mformer
...
class COSINE(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        id_prompts = torch.cat(id_prompt_list, dim=0)
        sem_prompts = torch.cat(sem_prompt_list, dim=0)
        id_prompt_labels = torch.cat(id_prompt_label_list, dim=0)
        sem_prompt_labels = torch.cat(sem_prompt_label_list, dim=0)

        new_id_prompts = []
        new_id_prompy_labels = []
        id_labels = torch.unique(id_prompt_labels)
        uni_id_label = id_labels.cpu().tolist()
        for id in uni_id_label:
            l_ids = id_prompt_labels == id
            id_prompt = id_prompts[l_ids]
            id_prompt = torch.mean(id_prompt, dim=0, keepdim=True)
            new_id_prompts.append(id_prompt)
            new_id_prompy_labels.append(id)
        id_prompts = torch.cat(new_id_prompts, dim=0)
        id_prompt_labels = torch.tensor(new_id_prompy_labels, dtype=torch.int64, device=id_prompts.device)


        new_sem_prompts = []
        new_sem_prompy_labels = []
        sem_labels = torch.unique(sem_prompt_labels)
        uni_sem_label = sem_labels.cpu().tolist()
        for sem in uni_sem_label:
            l_sems = sem_prompt_labels == sem
            sem_prompt = sem_prompts[l_sems]
            sem_prompt = torch.mean(sem_prompt, dim=0, keepdim=True)
            new_sem_prompts.append(sem_prompt)
            new_sem_prompy_labels.append(sem)
        sem_prompts = torch.cat(new_sem_prompts, dim=0)
        sem_prompt_labels = torch.tensor(new_sem_prompy_labels, dtype=torch.int64, device=sem_prompts.device)

        prompts = {
            'id_prompt_list': [id_prompts],
            'sem_prompt_list': [sem_prompts],
            'id_prompt_label_list': [id_prompt_labels],
            'sem_prompt_label_list': [sem_prompt_labels]
        }
