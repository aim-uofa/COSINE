...
from inference_vos.model.model_ms import build_model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-videos', type=int, default=None,
                help='Run only the first N videos for smoke testing.')
    parser.add_argument('--max-frames', type=int, default=None,
                help='Run only the first N frames in each video for smoke testing.')
    ...
    parser.add_argument('--weights', type=str, default="models/cosine/mscosine_vos_fd1_md6_lr1e-4_ep50/pytorch_model/pytorch_model_24ep/pytorch_model.bin")
    ...
    if args.max_videos is not None:
        max_vid = min(max_vid, args.max_videos)
    ...
    for i, vid_reader in tqdm(zip(range(max_vid), data_loader_val), total=min(max_vid, len(data_loader_val)), disable=not ddp_utils.is_main_process()):
        ...
        n_obj = 0
        for ti, data in enumerate(loader):
            if args.max_frames is not None and ti >= args.max_frames:
                break
