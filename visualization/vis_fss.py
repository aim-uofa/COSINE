...
from inference_fss.model.model_ms import build_model
...
def test(model, dataloader, args=None):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        if Visualizer.visualize:
            ...
            save_root = Visualizer.vis_path

            query_name = batch['query_name'][0].split('/')[-1].replace('.jpg', '')
            support_name = batch['support_names'][0][0].split('/')[-1].replace('.jpg', '')
            score = int(area_inter[1].float() / area_union[1].float() * 100)
            class_id = batch['class_id'][0].item()
            save_name = f"class{class_id}_score{score}_{support_name}_{query_name}"

            support_img = batch['support_imgs'][0][0]
            support_mask = batch['support_masks'][0][0]
            tgt_img = batch['query_img'][0]
            tgt_mask = batch['query_mask'][0]

            support_visualizer = MaskVisualizer(support_img.permute(1,2,0).cpu().numpy() * 255)
            tgt_visualizer = MaskVisualizer(tgt_img.permute(1,2,0).cpu().numpy() * 255)

            support_vis_output = support_visualizer.draw_binary_mask(
                support_mask.cpu().numpy(),
                color='deepskyblue',
                edge_color=_OFF_WHITE,
            )
            tgt_vis_output = tgt_visualizer.draw_binary_mask(
                tgt_mask.cpu().numpy(),
                color='lawngreen',
                edge_color=_OFF_WHITE,
            )
            ...
            tgt_vis_output.save(os.path.join(save_root, save_name+'_tgt.jpg'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--benchmark', type=str, default='lvis',
    ...
    parser.add_argument('--log-root', type=str, default='outputs/vis/fss/lvis_fold0')
