...
from . import build_clip, build_dinov2
...
class EncoderSoup(nn.Module):
    def __init__(
        ...
        if self.merging_type == 'simple_merging':
            ...
            #     LayerNorm2d(dim),
            # )
            self.merging_module = nn.Identity()

        elif self.merging_type == 'only_clip' or self.merging_type == 'only_dino':
    ...
    def merge_feature(self, features, prompts, batched_tag):
        ...
        if self.merging_type == 'simple_merging':
            ...
            embeddings = torch.cat([dino_embeddings, clip_embeddings], dim=1)
            embeddings = self.merging_module(embeddings)

        elif self.merging_type == 'no_merging':
            embeddings = []
            for dino_embed, clip_embed, tag in zip(dino_embeddings, clip_embeddings, batched_tag):
                if tag == 'refer':
                    embeddings.append(clip_embed + dino_embed * 0.)
                elif tag == 'sem':
                    embeddings.append(clip_embed + dino_embed * 0.)
                elif tag == 'visual':
                    embeddings.append(clip_embed * 0. + dino_embed)
                else:
                    raise NotImplementedError
            embeddings = torch.stack(embeddings, dim=0)
            embeddings = self.merging_module(embeddings)

        elif self.merging_type == 'only_clip':
            embeddings = dino_embeddings * 0. + clip_embeddings
            embeddings = self.merging_module(embeddings)

        elif self.merging_type == 'only_dino':
            embeddings = dino_embeddings + clip_embeddings * 0.
            embeddings = self.merging_module(embeddings)
...
def build_encodersoup(args):
    ...
    return EncoderSoup(
        ...
        clip,
        # sam
        merging_type=args.img_feats_merging_type
    )
