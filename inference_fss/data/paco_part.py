...
class DatasetPACOPart(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize, box_crop=True):
        ...
        paco_part_meta = MetadataCatalog.get('paco_lvis_v1_train')
        self.class_names = paco_part_meta.thing_classes
        self.thing_dataset_id_to_contiguous_id = paco_part_meta.thing_dataset_id_to_contiguous_id
    ...
    def __getitem__(self, idx):
        ...
        batch = {'query_img': query_img,
                 ...
                 'class_name': self.class_names[self.thing_dataset_id_to_contiguous_id[class_sample]]
                 }
...
if __name__ == '__main__':
    ...
    dataset = DatasetPACOPart(datapath, fold=fold, transform=transform, split=split, shot=1,
                                      use_original_imgsize=False)
    ...
    for idx in range(len(dataset)):
        ...
        if not os.path.exists(f'shows/pacopart/fold{fold}'):
            os.makedirs(f'shows/pacopart/fold{fold}')

        save_path = f'shows/pacopart/fold{fold}/{class_name}_{query_n}'
