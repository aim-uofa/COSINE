...
from . import build_dinov2_ms
from cosine.model.image_encoder.clip_ms_fsod import build_clip as build_clip_ms
...
class EncoderSoup(nn.Module):
    ...
    def encode_image(
        ...
        if len(clip_embeddings) == 5:
            clip_vis_dense = clip_embeddings[-1]
            clip_embeddings = [clip_embeddings[i] for i in range(len(clip_embeddings)-1)]
        else:
            clip_vis_dense = None
        dino_embeddings = self.dinov2.forward_neck(dino_embeddings)
        clip_embeddings = self.clip.forward_neck(clip_embeddings)
    ...
    def encode_prompt_combine_vis_text(self, batched_visual_prompt, batched_text_prompt, batched_tag):
        ...
        for vis_prompt, text_prompt, tag in zip(batched_visual_prompt, batched_text_prompt, batched_tag):
            ...
            assert text_prompt is not None, "text_prompt must be provided!"
            text_labels, texts = [], []
            for k in text_prompt:
                text_labels.append(k)
                texts.append(text_prompt[k])
            text_prompts = self.encode_text_prompt(texts)
            ...
            text_prompts = text_prompts[sorted_indices]

            sem_prompts = torch.cat([text_prompts, sem_queries], dim=0)
            sem_labels = torch.cat([text_labels, sem_labels])
    ...
    def encode_prompt(self, batched_visual_prompt, batched_text_prompt, batched_tag):
        ...
        for vis_prompt, text_prompt, tag in zip(batched_visual_prompt, batched_text_prompt, batched_tag):
            if tag == 'refer':
                ...
                id_prompts, id_labels = sem_prompts, sem_labels
    ...
    def merge_feature(self, features, prompts):
        ...
        assert len(dino_embeddings) == len(clip_embeddings)

        results = {}
        if self. merging_type == 'simple_merging':

            for layer in dino_embeddings:
                dino_embed = dino_embeddings[layer]
                clip_embed = clip_embeddings[layer]

                embeddings = torch.cat([dino_embed, clip_embed], dim=1)
...
def build_encodersoup(args):
    ...
    dinov2 = build_dinov2_ms(args)

    # SAM
    # sam = build_sam(args)

    # CLIP
    clip = build_clip_ms(args)
