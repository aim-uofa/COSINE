...
def transform_instance_annotations(
    ...
    if "segmentation" in annotation:
        ...
        if len(segm) == 0 or (isinstance(segm, list) and type(segm[0]) != dict):
            # polygons
            from pycocotools import mask as coco_mask
            rle = coco_mask.frPyObjects(
                segm, transforms.transforms[0].h, transforms.transforms[0].w
            )
            # RLE
            mask = mask_util.decode(rle)
            mask = mask.sum(-1)
            mask = transforms.apply_segmentation(mask.astype(dtype=np.float_))
...
def annotations_to_instances(annos, image_size, mask_format="polygon"):
    ...
    if len(annos) and "segmentation" in annos[0]:
        ...
        if mask_format == "polygon":
            ...
            masks = BitMasks(
                torch.stack([torch.from_numpy(np.ascontiguousarray(x.astype(dtype=np.float_))) for x in masks])
            )
