...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        datasets="pano_seg||ins_seg||||refer_seg",
        sample_rate=[1, 1, 1],
        pano_seg_data="coco",
        pano_sample_rate=[1,],
        ins_seg_data="o365||paco",
        ins_sample_rate=[1, 1,],
        ...
        self.seg_dataset_dict = {
            'pano_seg': pano_seg_data.split("||") if 'pano_seg' in datasets else None,
        ...
        self.dataset_sample_rate_dict = {
            'pano_seg': pano_sample_rate / pano_sample_rate.sum() if 'pano_seg' in datasets else None,
        ...
        multimodal_rate = np.array(multimodal_rate)
        self.multimodal_rate = multimodal_rate / multimodal_rate.sum()
    ...
    def __getitem__(self, idx):
        ...
        if 'pano' in seg_flag or 'ins' in seg_flag:
            modal = np.random.choice(self.multimodal_choice, p=self.multimodal_rate)
            dataset.set_multimodal(visual_prompt='visual' in modal, text_prompt='text' in modal)
        data = dataset[idx]
        return data
...
def build_dataset(args, is_train):
    ...
    dataset = HybridDataset(
        ...
        multimodal_rate=[float(x) for x in args.multimodal_rate.split(",")],
...
if __name__ == '__main__':
    ...
    for id in range(len(dataset)):
        ...
        if visual_prompt is not None and len(visual_prompt['instances'])>0:
            print(f"iter: {id} ", visual_prompt['file_name'], f"ins num: {len(visual_prompt['instances'])}")
        else:
            print()

        if len(target['instances'])>0:
            print(f"iter: {id} ", target['file_name'], f"ins num: {len(target['instances'])}")
        else:
            print()